...
function Export-MWPrinters {
    ...
    if (-not (Test-Path -LiteralPath $DestinationFolder)) {
        try {
            ...
            Write-MWLogInfo "Dossier d'export imprimantes créé : $DestinationFolder"
        } catch {
            Write-MWLogError "Impossible de créer le dossier d'export imprimantes '$DestinationFolder' : $_"
            throw
        }
    }
    ...
    if (Get-Command Get-Printer -ErrorAction SilentlyContinue) {
        try {
            ...
            Write-MWLogInfo "Liste imprimantes exportée -> $printersJsonPath"
            ...
            if ($def) {
                ...
                Write-MWLogInfo "Imprimante par défaut exportée : $def"
            } else {
    ...
    if (Get-Command Get-PrinterPort -ErrorAction SilentlyContinue) {
        try {
            ...
            Write-MWLogInfo "Ports d'imprimantes exportés -> $portsJsonPath"
        } catch {
...
function Import-MWPrinters {
    ...
    # Import des ports TCP/IP en priorité
        ...
        try {
            ...
            foreach ($pr in $plist) {
                ...
                if (-not $name -or -not $drv -or -not $port) {
                    Write-MWLogWarning "Imprimante ignorée (informations incomplètes) : Name='$name', Driver='$drv', Port='$port'"
                    continue
                }

                if (-not (Get-PrinterPort -Name $port -ErrorAction SilentlyContinue)) {
                    Write-MWLogWarning "Imprimante '$name' ignorée : port '$port' introuvable (WSD/USB ou partagé ?)."
                    continue
                }

                if (-not (Get-Printer -Name $name -ErrorAction SilentlyContinue)) {
                    try {
                        Write-MWLogInfo "Création de l'imprimante '$name' (driver='$drv', port='$port')."
                        ...
                        Write-MWLogError "Création de l'imprimante '$name' échouée : $_"
                    }
                } else {
                    Write-MWLogInfo "Imprimante déjà existante, non recréée : $name"
                }
            }
            ...
            if (Test-Path -LiteralPath $defPath -PathType Leaf) {
                ...
                if ($def -and (Get-Printer -Name $def -ErrorAction SilentlyContinue)) {
                    try {
                        ...
                        Write-MWLogInfo "Imprimante par défaut définie : $def"
                    } catch {
                        Write-MWLogWarning "Impossible de définir l'imprimante par défaut '$def' : $_"
                    }
                } else {
                    Write-MWLogWarning "Imprimante par défaut '$def' introuvable après import."
                }
            }
        } catch {
